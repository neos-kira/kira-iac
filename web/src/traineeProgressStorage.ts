...
export type TraineeProgressSnapshot = {
  ...
  /** 課題5: 完了済みフェーズ番号一覧（1〜5）※後方互換 */
  infra5PhaseDone?: number[]
  /** 課題5: サーバー構築実践で完了済みの問題番号一覧 ※後方互換 */
  infra5BuildDone?: number[]
  /** 課題5: トラブルシューティングで完了済みの問題番号一覧 ※後方互換 */
  infra5TroubleDone?: number[]
  /** 課題5: セキュリティチェックで完了済みの問題番号一覧 ※後方互換 */
  infra5SecDone?: number[]
  /** 課題5（新）: チェックボックス状態（40項目） */
  infra5Checkboxes?: boolean[]
  /** 課題5（新）: セクション完了状態（s1〜s5） */
  ...
  /** IT業界の歩き方: カテゴリIDごとの合格状態 */
  itBasicsPassed?: Record<string, boolean>
}
