...
function CodeBlock({ code }: { code: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-3">
      <code translate="no" className="flex-1 text-body md:text-body-pc font-mono text-emerald-400 break-all">{code}</code>
          ...
          <>
            ...
            <section className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-label md:text-label-pc">
                <div>
                  ...
                  <p translate="no" className="font-mono font-semibold text-slate-800">{ip}</p>
                </div>
                <div>
                  <span className="text-slate-400">ユーザー名</span>
                  <p translate="no" className="font-mono font-semibold text-slate-800">{user}</p>
                </div>
                <div className="col-span-2">
                  <span className="text-slate-400">秘密鍵ファイル</span>
                  <p translate="no" className="font-mono text-slate-700">{pemFile}</p>
                </div>
              </div>
            ...
            <section className={`rounded-xl border bg-white shadow-sm overflow-hidden transition-colors ${sshPassed ? 'border-emerald-300' : 'border-slate-200'}`}>
              ...
              <div className="flex items-center justify-between px-4 pt-4 pb-3">
                ...
                {sshPassed && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-1 text-label md:text-label-pc font-semibold text-emerald-700">
                    ✓ 完了
                  </span>
                )}
              </div>
              ...
              <div className="px-4 pb-4 space-y-4">
                {/* OS タブ */}
                <div className="flex rounded-lg border border-slate-200 overflow-hidden text-label md:text-label-pc font-medium w-fit">
                  ...
                  <ol className="space-y-3">
                    ...
                    <li className="flex gap-3">
                      ...
                      <div className="flex-1 pt-0.5">
                        ...
                        <p className="mt-1.5 text-label md:text-label-pc text-slate-600">「Are you sure you want to continue connecting?」と表示されたら <code translate="no" className="bg-slate-100 px-1 rounded">yes</code> と入力してEnter</p>
                      </div>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <div className="flex-1 pt-0.5">
                        <p className="text-body md:text-body-pc text-slate-700">プロンプト（<code translate="no" className="bg-slate-100 px-1 rounded text-label md:text-label-pc">{user}@ip-xxx-xxx-xxx-xxx:~$</code>）が表示されれば接続成功</p>
                      </div>
                    </li>
                  </ol>
                )}
                  ...
                  <ol className="space-y-3">
                    <li className="flex gap-3">
                      ...
                      <div className="flex-1 pt-0.5">
                        ...
                        <p className="text-label md:text-label-pc text-slate-600">スタートメニューで「PowerShell」と検索して起動<br/>または Windowsキー + R → <code translate="no" className="bg-slate-100 px-1 rounded">powershell</code> → Enter</p>
                      </div>
                    </li>
                    <li className="flex gap-3">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-label md:text-label-pc font-bold text-slate-600">2</span>
                ...
                <div className="pt-2 border-t border-slate-100">
                  ...
                  <button
                    ...
                    disabled={isGrading}
                    className="w-full rounded-xl bg-sky-600 py-2.5 text-button md:text-button-pc font-semibold text-white hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {isGrading ? 'AI採点中...' : 'スクリーンショットをアップロード'}
                  </button>

                  {gradeState.ssh && (
                    <div className={`mt-2.5 rounded-xl border px-4 py-3 text-body md:text-body-pc font-medium ${gradeState.ssh.passed ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}>
            ...
            {allPassed && (
              <section className="rounded-xl border-2 border-emerald-300 bg-emerald-50 px-5 py-4 shadow-sm">
                <p className="text-body md:text-body-pc font-bold text-emerald-800">SSH接続演習が完了しました 🎉</p>
