...
### ① データの正はDynamoDB
どの端末・どのブラウザからログインしても
同じ状態が見えなければならない。
localStorageはブラウザ固有のため
正のデータソースにしてはならない。

### ② テナント分離
会社Aのデータが会社Bに漏れてはならない。
全てのAPIリクエストにtenantIdを含め
DynamoDBのクエリは必ずtenantIdでフィルタリングする。

### ③ 状態の一貫性
localStorageはキャッシュに過ぎない。
DynamoDBの値とlocalStorageが食い違ったら
必ずDynamoDBを優先する。
localStorageから読んでDynamoDBに書くことは禁止。

### ④ 操作→保存→表示の順序（絶対遵守）
ユーザーが操作する
↓
DynamoDBに即時保存（await完了まで待つ）
↓
画面の状態を更新する

この順序を絶対に崩さない。
保存完了前に画面を更新したり遷移したりしてはならない。

### ⑤ 中断・再開はどの端末でも動く
スマホで中断してPCで再開しても
同じ場所から始まらなければならない。
これはDynamoDBに正しく保存できている場合のみ実現できる。
localStorageに依存した実装は
この原則に違反している。
...
## 修正作業の報告形式
修正作業を行う際は、必ず以下の形式で報告すること。

### 1. 原因
何が問題だったかを1〜3行で明記する。
例:「DynamoDBにintroStepが保存されていないユーザーが存在し、
undefinedとして扱われてバナー非表示条件を満たさなかった」

### 2. 修正したファイルと変更内容
ファイル名と具体的に何を変えたかを記載する。

### 3. 未修正のファイルと残タスク
まだ修正していないファイルと残タスクを記載する。

### 4. ビルド・デプロイ結果
型エラー・ビルドエラーの有無とデプロイ結果を記載する。
