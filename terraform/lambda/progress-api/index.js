...
返答は必ず2文以内に収めること。3文以上は絶対禁止。`
    ...
    if (method === 'GET' && (path === '/auth/me' || path === '/auth/me/')) {
      ...
      return json({ username: session.username, role: session.role || 'student', termsAgreedAt, termsVersion, accountType, displayName, email })
    }
    ...
    if (method === 'GET' && (path === '/api/profile' || path === '/api/profile/')) {
      const session = await verifySession(event)
      if (!session) return json({ error: 'unauthorized' }, 401)
      if (!AccountsTableName) return json({ error: 'server error' }, 500)
      const profRes = await client.send(new GetItemCommand({
        TableName: AccountsTableName,
        Key: marshall({ username: session.username }),
      }))
      if (!profRes.Item) return json({ displayName: null, email: null })
      const profAcc = unmarshall(profRes.Item)
      return json({ displayName: profAcc.displayName || null, email: profAcc.email || null })
    }

    // プロフィール更新（PUT /api/profile）
    if (method === 'PUT' && (path === '/api/profile' || path === '/api/profile/')) {
      const session = await verifySession(event)
      if (!session) return json({ error: 'unauthorized' }, 401)
      if (!AccountsTableName) return json({ error: 'server error' }, 500)
      const body = JSON.parse(event.body || '{}')
      const newDisplayName = (typeof body.displayName === 'string' ? body.displayName : '').trim()
      if (!newDisplayName) return json({ error: 'displayName is required' }, 400)
      const newEmail = typeof body.email === 'string' ? body.email.trim() : ''
      const updateExpr = newEmail ? 'SET #dn = :d, email = :e' : 'SET #dn = :d'
      const exprAttrNames = { '#dn': 'displayName' }
      const exprAttrVals = newEmail ? { ':d': newDisplayName, ':e': newEmail } : { ':d': newDisplayName }
      await client.send(new UpdateItemCommand({
        TableName: AccountsTableName,
        Key: marshall({ username: session.username }),
        UpdateExpression: updateExpr,
        ExpressionAttributeNames: exprAttrNames,
        ExpressionAttributeValues: marshall(exprAttrVals),
      }))
      return json({ ok: true, displayName: newDisplayName, email: newEmail || null })
    }
    ...
    // EC2個別停止（POST /admin/ec2/stop）- managerのみ
    if (method === 'POST' && (path === '/admin/ec2/stop' || path === '/admin/ec2/stop/')) {
      const session = await verifySession(event)
      if (!session) return json({ error: 'unauthorized' }, 401)
      const role = await getSessionRole(session)
      if (role !== 'manager') return json({ error: 'forbidden' }, 403)
      ...
      if (!instanceId || typeof instanceId !== 'string') return json({ error: 'instanceId required' }, 400)

      // AWS EC2 実停止
      try {
        await ec2Client.send(new StopInstancesCommand({ InstanceIds: [instanceId] }))
      } catch (ec2Err) {
        console.error(JSON.stringify({ level: 'error', event: 'ec2_stop_failed', instanceId, error: ec2Err.message }))
        return json({ error: 'EC2停止に失敗しました: ' + ec2Err.message }, 500)
      }

      // DynamoDB の ec2State を stopped に更新（ec2InstanceId で対象ユーザーを検索）
      const { Items: allItems } = await client.send(new ScanCommand({
        TableName,
        FilterExpression: 'ec2InstanceId = :iid',
        ExpressionAttributeValues: marshall({ ':iid': instanceId }),
      }))
      const target = (allItems || []).map((i) => unmarshall(i))[0]
      if (target) {
        await client.send(new UpdateItemCommand({
          TableName,
          Key: marshall({ traineeId: target.traineeId }),
          UpdateExpression: 'SET ec2State = :state, updatedAt = :ts',
          ExpressionAttributeValues: marshall({ ':state': 'stopped', ':ts': new Date().toISOString() }),
        }))
      }

      console.log(JSON.stringify({ level: 'info', event: 'ec2_individual_stop', instanceId, traineeId: target?.traineeId ?? 'unknown', timestamp: new Date().toISOString() }))
      return json({ ok: true })
    }
    ...
    if (method === 'POST' && (path === '/admin/ec2/stop-all' || path === '/admin/ec2/stop-all/')) {
      ...
      const { Items } = await client.send(new ScanCommand({ TableName }))
      const running = (Items || []).map((i) => unmarshall(i)).filter((t) => t.ec2State === 'running' && t.ec2InstanceId)
      let stoppedCount = 0
      await Promise.allSettled(running.map(async (t) => {
        try {
          // AWS EC2 実停止
          await ec2Client.send(new StopInstancesCommand({ InstanceIds: [t.ec2InstanceId] }))
          // DynamoDB 更新
          await client.send(new UpdateItemCommand({
            TableName,
            Key: marshall({ traineeId: t.traineeId }),
            UpdateExpression: 'SET ec2State = :state, updatedAt = :ts',
            ExpressionAttributeValues: marshall({ ':state': 'stopped', ':ts': new Date().toISOString() }),
          }))
          stoppedCount++
        } catch (err) {
          console.error(JSON.stringify({ level: 'error', event: 'ec2_stop_all_item_failed', instanceId: t.ec2InstanceId, error: err.message }))
        }
      }))
      console.log(JSON.stringify({ level: 'info', event: 'ec2_stop_all', stoppedCount, timestamp: new Date().toISOString() }))
