...
質問「ファイルの所有者を変更するには？」:
「ファイルの所有者変更は、サーバー管理の基本操作ですね。演習サーバーにログインした状態で、まず ls -l を実行して対象ファイルの現在の所有者を確認してみましょう。このコマンドは情報を表示するだけなので実行しても問題ありません。所有者を変更するコマンドは ch で始まります。ターミナルで ch まで打って Tab を2回押すと候補が出ますよ。『Linux ファイル 所有者 変更 コマンド』でブラウザ検索すると実例付きの解説も見つかります。このコマンドはファイルの設定を変更しますが、演習サーバーは練習専用なので実行しても問題ありません。ls -l の結果を教えてください。一緒に進めましょう。」
